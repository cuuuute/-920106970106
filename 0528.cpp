...
pigInf* sellPig(pigInf *&head){										//每过三个月（默认一个月三十天,一年360天）
	...
	ofstream ofs;													//创建对象 
	ofs.open("pig_out.txt",ios::out);								//打开文件（文件路径/文件名，打开方式/写文件） 
	if(head->getTime()>=12||head->getWeight()>=75){
			if(head->getType()==0){
				balance+=b*head->getWeight();
				ofs<<head->getYear()<<'.'<<head->getMonth()<<'.'<<head->getDay()<<"  出售黑猪一只"<<endl;
			} 
			else if(head->getType()==1){
				balance+=f*head->getWeight();
				ofs<<head->getYear()<<'.'<<head->getMonth()<<'.'<<head->getDay()<<"  出售小花猪一只"<<endl;
			}
			else{
				balance+=w*head->getWeight();
				ofs<<head->getYear()<<'.'<<head->getMonth()<<'.'<<head->getDay()<<"  出售大白花猪一只"<<endl;
			} 
			p=head;
			head=head->next;
			delete p;
	...
	while(p1->next!=NULL){	
		if(p1->getTime()>=12||p1->getWeight()>=75){
			if(p1->getType()==0){
				...
				ofs<<p1->getYear()<<'.'<<p1->getMonth()<<'.'<<p1->getDay()<<"  出售黑猪一只"<<endl;
			} 
			else if(p1->getType()==1){
				balance+=f*p1->getWeight();
				ofs<<p1->getYear()<<'.'<<p1->getMonth()<<'.'<<p1->getDay()<<"  出售小花猪一只"<<endl;
			}
			else{
				balance+=w*p1->getWeight();
				ofs<<p1->getYear()<<'.'<<p1->getMonth()<<'.'<<p1->getDay()<<"  出售大白花猪一只"<<endl;
			}
			p2=p1;
			p1=p1->next;
			delete p2;
...
void searchRecord_output(pigInf *&head){
	ifstream ifs;																	//创建对象 
	ifs.open("pig_output.txt",ios::in);												//打开文件（文件路径/文件名，打开方式/读文件） 
	if(!ifs.is_open()){																//利用成员函数判断是否打开成功 
		cout<<"Open Failure!";
		system("pause");
	}
...
//	pigInf pig[10];
	menu(head);
	if((!Day%90)&&Day!=0) sellPig(head);							//每3个月举行一次卖猪猪活动 
